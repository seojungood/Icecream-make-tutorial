    ...
    , ui(new Ui::MainWindow)
{
    ...
    QGraphicsPixmapItem *item = new QGraphicsPixmapItem(
        QPixmap(":/Resources/Sprites/kitchen.jpg").scaled(800, 600));
    scene->addItem(item);
    ...
    QGraphicsPixmapItem *background = new QGraphicsPixmapItem(
        QPixmap(":/Resources/Sprites/endScreen.png").scaled(800, 600));
    ...
    connect(ui->screen02Boil,
            &Boil::boilSucceeded,
            &model,
            &Model::incrementScreen); // Move from Boil to Freeze

    // Screen switching connections
    connect(ui->screen04Churn,
            &Churn::churningComplete,
            &model,
            &Model::incrementScreen); // Move from Churn to End Screen
    connect(ui->screen04Churn,
            &Churn::churningComplete,
            this,
            &MainWindow::on_change_to_End_Screen); // Shoot confetti
    ...
    connect(&model, &Model::openFridgeSignal, this, &MainWindow::openFridge);
}
...
void MainWindow::updateRects(std::vector<b2Body *> bodies)
{
    for (int i = 0; i < bodies.size(); ++i) {
        ...
        graphicsRects[i]->setPos(300 + position.x * 40,
                                 600
                                     + (-position.y
                                        * 50)); // Transform from world to Graphics view coordinates
